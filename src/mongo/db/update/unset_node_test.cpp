...
TEST_F(UnsetNodeTest, UnsetArrayElement) {
    ...
    ASSERT_FALSE(doc.isInPlaceModeEnabled());
    if (v2LogBuilderUsed()) {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.0': true}}"), getOplogEntry());
    } else {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.0': true}}"), getOplogEntry());
    }
...
TEST_F(UnsetNodeTest, UnsetPositional) {
    ...
    ASSERT_FALSE(doc.isInPlaceModeEnabled());
    if (v2LogBuilderUsed()) {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.1': true}}"), getOplogEntry());
    } else {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.1': true}}"), getOplogEntry());
    }
...
TEST_F(UnsetNodeTest, UnsetFromObjectInArray) {
    ...
    ASSERT_FALSE(doc.isInPlaceModeEnabled());
    if (v2LogBuilderUsed()) {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.0.b': true}}"), getOplogEntry());
    } else {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.0.b': true}}"), getOplogEntry());
    }
...
TEST_F(UnsetNodeTest, ApplyCanRemoveRequiredPartOfDBRefIfValidateForStorageIsFalse) {
    ...
    ASSERT_FALSE(doc.isInPlaceModeEnabled());
    if (v2LogBuilderUsed()) {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.$id': true}}"), getOplogEntry());
    } else {
        ASSERT_BSONOBJ_EQ(fromjson("{$unset: {'a.$id': true}}"), getOplogEntry());
    }
