...
public:
    ...
    const std::vector<FieldRef>& toRemove() const {
        return _toRemove;
    }

    const std::vector<std::pair<FieldRef, BSONElement>> toSet() const {
        return _toSet;
    }
    ...
    size_t computeApproxSize() const {
        size_t sum = 0;
        for (auto&& f : _toRemove) {
            sum += f.dottedField().size();
        }

        for (auto&& [fr, elt] : _toSet) {
            sum += fr.dottedField().size();
            sum += elt.size();
        }
        return sum;
    }
...
private:
    ...
    std::vector<FieldRef> _toRemove;
    std::vector<std::pair<FieldRef, BSONElement>> _toSet;
};
