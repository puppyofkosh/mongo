...
/**
 * There are multiple types of 'u' (update) oplog entries. The type of an entry is indicated using a
 * field called $v.
 *
...
enum class UpdateOplogEntryVersion {
    ...
    // post images are diffed, producing a delta. The delta is recorded in the oplog. On secondaries,
    // the delta is applied to the pre-image to recover the post image.
    //
