...
TEST(V2LogBuilder, UpdateFieldMutableBson) {
    mmb::Document doc;
    // Element is an array.
    const mmb::Element eltArr = doc.makeElementArray("a.b",
                                                     BSON_ARRAY(1 << BSON("sub"
                                                                          << "obj")
                                                                  << 3));
    // Element is an obj.
    const mmb::Element eltObj = doc.makeElementObject("c.d", fromjson("{arr: [{a: [0]}]}"));
    ...
    ASSERT_BSONOBJ_BINARY_EQ(
        mongo::fromjson("{ $v: 2, diff: { sa: {u: {b: [1, {sub: 'obj'}, 3]}}, sc: {u: {d: "
                        "{arr: [{a: [0]}]}, e: 'val'}} }}"),
        lb.serialize());
}
...
TEST(V2LogBuilder, ArrayModifications) {
    ...
    ASSERT_OK(lb.logCreatedField(
        makeRuntimeUpdatePathAssumeNumericComponentsAreIndexes("a.10.insert.a"), 2, elt_cd));
    ASSERT_OK(
        lb.logUpdatedField(makeRuntimeUpdatePathAssumeNumericComponentsAreIndexes("a.3"), elt_ab));
    ASSERT_OK(lb.logCreatedField(
        makeRuntimeUpdatePathAssumeNumericComponentsAreIndexes("a.0.c.insert.a"), 3, elt_cd));
    ASSERT_OK(lb.logCreatedField(
        makeRuntimeUpdatePathAssumeNumericComponentsAreIndexes("a.0.c.insert.f"), 3, elt_cd));
    ASSERT_OK(
    ...
    ASSERT_BSONOBJ_BINARY_EQ(
        mongo::fromjson("{$v: 2, diff: {sa: {a: true, s0: {sc: {d: {del: false}, u: {update: 1}, "
                        "i: {insert: {a: 2, f: 2}}}}, u3: 1, s10: {i: {insert: {a: 2}}} }}}"),
        lb.serialize());
}
...
TEST(V2LogBuilder, ImplicityArrayCreationDisallowed) {
    mmb::Document doc;
    V2LogBuilder lb;
