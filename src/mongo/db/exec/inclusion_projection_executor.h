...
// TODO: Consider breaking this off so the inheritance tree looks like
//          ProjectionNode
//    ArrayNode         ProjectionIncExc
//                   InclusionNode       ExclusionNode
//
class ArrayProjectionNode : public ProjectionNode {
public:
    ArrayProjectionNode(ProjectionPolicies policies, std::string pathToNode = "")
    ...
    void reportDependencies(DepsTracker* deps) const final {
        ...
        // TODO: Add children dependencies in case they have expressions that need metadata.
    }

    // TODO in new inheritance hierchy this shouldn't be here.
    boost::optional<size_t> maxFieldsToProject() const override {
        MONGO_UNREACHABLE;
    }

    ProjectionNode* addChild(size_t index, std::unique_ptr<ProjectionNode> child) {
        _children.push_back(std::make_pair(index, std::move(child)));
        return _children.back().second.get();
    }
...
protected:
    ...
    Value applyExpressionsToValue(const Document& root, Value inputVal) const override final {
        std::cout << "ian applyExpressionsToValue: input is " << inputVal << std::endl;
        if (inputVal.missing()) {
            return Value();
        }
        // TODO: Will this be invariant or uassert
        invariant(inputVal.getType() == BSONType::Array);

        // TODO: How do we avoid copying the entire array when modifying it?
        // We can only modify the Value itself when it's exclusively owned...but we pass Values by
        // value all the time :(

        auto vec = inputVal.getArray();
        for (auto&& pair : _children) {
            if (pair.first < vec.size()) {
                vec[pair.first] = pair.second->applyExpressionsToValue(root, vec[pair.first]);
            }
        }

        return Value(vec);
        // TODO: what happens if we have things that go past the end?
    }

    Value applyProjectionsToValue(Value inputVal) const override final {
        if (inputVal.missing()) {
            return Value();
        }
        // TODO: Will this be invariant or uassert
        std::cout << "ian applyProjectionsToValue: input is " << inputVal << std::endl;
        invariant(inputVal.getType() == BSONType::Array);
        ...
        for (auto&& pair : _children) {
            if (pair.first < vec.size()) {
                vec[pair.first] = pair.second->applyProjectionsToValue(vec[pair.first]);
            }
        }
...
private:
    // TODO: Maybe a different data structure
    std::vector<std::pair<size_t, std::unique_ptr<ProjectionNode>>> _children;
