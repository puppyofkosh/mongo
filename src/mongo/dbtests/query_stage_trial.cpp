...
TEST_F(TrialStageTest, AdoptsTrialPlanIfTrialSucceeds) {
    auto trialPlan = std::make_unique<QueuedDataStage>(_expCtx, ws());
    auto backupPlan = std::make_unique<QueuedDataStage>(_expCtx, ws());
    ...
    auto trialStage = std::make_unique<TrialStage>(
        _expCtx, ws(), std::move(trialPlan), std::move(backupPlan), 10, 0.75);
