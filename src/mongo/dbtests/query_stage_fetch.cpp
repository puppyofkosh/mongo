...
public:
    void run() {
        ...
        auto mockStage = std::make_unique<QueuedDataStage>(_expCtx, &ws);
        ...
        auto fetchStage =
            std::make_unique<FetchStage>(_expCtx, &ws, std::move(mockStage), nullptr, coll);
...
public:
    void run() {
        ...
        auto fetchStage = std::make_unique<FetchStage>(
            _expCtx, &ws, std::move(mockStage), filterExpr.get(), coll);
