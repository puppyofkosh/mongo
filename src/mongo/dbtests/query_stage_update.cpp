...
public:
    ...
    void getCollContents(Collection* collection, vector<BSONObj>* out) {
        ...
        unique_ptr<CollectionScan> scan(
            new CollectionScan(_expCtx, collection, params, &ws, nullptr));
        while (!scan->isEOF()) {
            WorkingSetID id = WorkingSet::INVALID_ID;
...
public:
    void run() {
        // Run the update.
        {
            ...
            auto expCtx = make_intrusive<ExpressionContext>(&_opCtx, collator);
            ...
            auto eofStage = make_unique<EOFStage>(expCtx);

            auto updateStage =
                make_unique<UpsertStage>(expCtx, params, ws.get(), collection, eofStage.release());
...
public:
    void run() {
        // Run the update.
        {
            ...
            auto ws = make_unique<WorkingSet>();
            auto cs =
                make_unique<CollectionScan>(expCtx, coll, collScanParams, ws.get(), cq->root());

            auto updateStage =
                make_unique<UpdateStage>(expCtx, updateParams, ws.get(), coll, cs.release());
...
public:
    void run() {
        ...
        // RID_AND_OBJ state.
        auto qds = make_unique<QueuedDataStage>(expCtx, ws.get());
        ...
        const auto updateStage =
            make_unique<UpdateStage>(expCtx, updateParams, ws.get(), coll, qds.release());
