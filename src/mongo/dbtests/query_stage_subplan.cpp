...
protected:
    ...
    boost::intrusive_ptr<ExpressionContext> _expCtx = new ExpressionContext(_opCtx.get(), nullptr);
...
TEST_F(QueryStageSubplanTest, QueryStageSubplanGeo2dOr) {
    ...
    std::unique_ptr<SubplanStage> subplan(
        new SubplanStage(_expCtx, collection, &ws, plannerParams, cq.get()));
...
TEST_F(QueryStageSubplanTest, QueryStageSubplanDontCacheZeroResults) {
    ...
    // cache entries).
    ws.clear();
    subplan.reset(new SubplanStage(_expCtx, collection, &ws, plannerParams, cq.get()));
...
TEST_F(QueryStageSubplanTest, ShouldReportErrorIfExceedsTimeLimitDuringPlanning) {
    ...
    SubplanStage subplanStage(
        _expCtx, ctx.getCollection(), &workingSet, params, canonicalQuery.get());
...
TEST_F(QueryStageSubplanTest, ShouldThrowOnRestoreIfIndexDroppedBeforePlanSelection) {
    ...
    // Create the SubplanStage.
    WorkingSet workingSet;
    SubplanStage subplanStage(_expCtx, collection, &workingSet, params, canonicalQuery.get());
