...
public:
    void run() {
        ...
        WorkingSet ws;
        auto ah = std::make_unique<AndHashStage>(_expCtx, &ws);

        // Foo <= 20.
        auto params = makeIndexScanParams(&_opCtx, getIndex(BSON("foo" << 1), coll));
        params.bounds.startKey = BSON("" << 20);
        params.direction = -1;
        ah->addChild(std::make_unique<IndexScan>(_expCtx, params, &ws, nullptr));
...
public:
    void run() {
        ...
        WorkingSet ws;
        auto ah = std::make_unique<AndHashStage>(_expCtx, &ws, 20 * big.size());
...
public:
    void run() {
        ...
        WorkingSet ws;
        auto ah = std::make_unique<AndHashStage>(_expCtx, &ws, 5 * big.size());
...
public:
    void run() {
        ...
        WorkingSet ws;
        auto ah = std::make_unique<AndHashStage>(_expCtx, &ws, 10 * big.size());
...
public:
    void run() {
        ...
        params.bounds.startKey = BSON("" << 20);
        params.direction = -1;
        auto firstScan = std::make_unique<IndexScan>(_expCtx, params, &ws, nullptr);

        // First child of the AND_HASH stage is a Fetch. The NULL in the
        // constructor means there is no filter.
        auto fetch =
            std::make_unique<FetchStage>(_expCtx, &ws, std::move(firstScan), nullptr, coll);
...
public:
    void run() {
        ...
        params = makeIndexScanParams(&_opCtx, getIndex(BSON("bar" << 1), coll));
        params.bounds.startKey = BSON("" << 10);
        auto secondScan = std::make_unique<IndexScan>(_expCtx, params, &ws, nullptr);

        // Second child of the AND_HASH stage is a Fetch. The NULL in the
        // constructor means there is no filter.
        auto fetch =
            std::make_unique<FetchStage>(_expCtx, &ws, std::move(secondScan), nullptr, coll);
        ah->addChild(std::move(fetch));
...
public:
    void run() {
        ...
        {
            WorkingSet ws;
            const auto andHashStage = std::make_unique<AndHashStage>(_expCtx, &ws);

            auto childStage1 = std::make_unique<QueuedDataStage>(_expCtx, &ws);
            {
            ...
            auto childStage2 = std::make_unique<QueuedDataStage>(_expCtx, &ws);
...
public:
    void run() {
        ...
        WorkingSet ws;
        auto ah = std::make_unique<AndSortedStage>(_expCtx, &ws);
...
public:
    void run() {
        ...
        WorkingSet ws;
        unique_ptr<AndSortedStage> as = std::make_unique<AndSortedStage>(_expCtx, &ws);
        ...
        params.bounds.startKey = BSON("" << 1);
        params.bounds.endKey = BSON("" << 1);
        as->addChild(std::make_unique<IndexScan>(_expCtx, params, &ws, nullptr));
