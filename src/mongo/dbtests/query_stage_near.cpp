...
public:
    void setUp() override {
        _expCtx = make_intrusive<ExpressionContext>(_opCtx, nullptr);
...
public:
    ...
    MockNearStage(const boost::intrusive_ptr<ExpressionContext>& expCtx,
        ...
        : NearStage(
              expCtx, "MOCK_DISTANCE_SEARCH_STAGE", STAGE_UNKNOWN, workingSet, indexDescriptor),
          _pos(0) {}
    ...
    virtual StatusWith<CoveredInterval*> nextInterval(OperationContext* opCtx,
        ...
        auto queuedStage = std::make_unique<QueuedDataStage>(_expCtx, workingSet);
...
TEST_F(QueryStageNearTest, Basic) {
    ...
    MockNearStage nearStage(_expCtx, &workingSet, _mockGeoIndex);
