...
public:
    ...
    unique_ptr<PlanExecutor, PlanExecutor::Deleter> makeCollScanExec(
        ...
        unique_ptr<PlanStage> root(
            new CollectionScan(cq->getExpCtx(), coll, csparams, ws.get(), cq.get()->root()));
    ...
    unique_ptr<PlanExecutor, PlanExecutor::Deleter> makeIndexScanExec(Database* db,
        ...
        unique_ptr<WorkingSet> ws(new WorkingSet());
        auto ixscan = std::make_unique<IndexScan>(_expCtx, ixparams, ws.get(), nullptr);
        unique_ptr<PlanStage> root =
            std::make_unique<FetchStage>(_expCtx, ws.get(), std::move(ixscan), nullptr, coll);
...
TEST_F(PlanExecutorTest, DropIndexScanAgg) {
    ...
    // in the pipeline.
    innerExec->saveState();
    auto cursorSource = DocumentSourceCursor::create(collection, std::move(innerExec), _expCtx);
    auto pipeline = assertGet(Pipeline::create({cursorSource}, _expCtx));

    // Create the output PlanExecutor that pulls results from the pipeline.
    auto ws = std::make_unique<WorkingSet>();
    auto proxy = std::make_unique<PipelineProxyStage>(_expCtx, std::move(pipeline), ws.get());
