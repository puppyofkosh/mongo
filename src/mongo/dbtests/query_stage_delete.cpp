...
public:
    void run() {
        ...
        WorkingSet ws;
        DeleteStage deleteStage(_expCtx,
                                std::move(deleteStageParams),
                                &ws,
                                coll,
                                new CollectionScan(_expCtx, coll, collScanParams, &ws, nullptr));
...
public:
    void run() {
        ...
        // RID_AND_OBJ state.
        auto qds = std::make_unique<QueuedDataStage>(_expCtx, ws.get());
        ...
        const auto deleteStage = std::make_unique<DeleteStage>(
            _expCtx, std::move(deleteParams), ws.get(), coll, qds.release());
