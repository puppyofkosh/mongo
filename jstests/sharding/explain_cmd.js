...
(function() {
    ...
    assert.commandFailedWithCode(explain, ErrorCodes.IllegalOperation);
    ...
    assert.commandFailedWithCode(explain, ErrorCodes.InvalidLength);
    ...
    assert.commandFailedWithCode(explain, ErrorCodes.ShardKeyNotFound);
    ...
    for (let shardExplain of explain.queryPlanner.winningPlan.shards) {
        ...
        assert.eq(
            shardExplain.winningPlan.transformBy.$sortKey.$meta, "sortKey", () => tojson(explain));
    }
    ...
    for (let shardExplain of explain.queryPlanner.winningPlan.shards) {
        ...
        assert.eq(
            shardExplain.winningPlan.inputStage.stage, "SHARDING_FILTER", () => tojson(explain));
    ...
    for (let shardExplain of explain.queryPlanner.winningPlan.shards) {
        ...
        // because the skip will be applied on the mongos.
        assert.eq(shardExplain.winningPlan.stage, "LIMIT", () => tojson(explain));
